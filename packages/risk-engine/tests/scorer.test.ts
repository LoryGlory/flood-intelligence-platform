...
import type { GaugeReading, StationConfig, WeatherForecast } from "@flood/core";
...
function makeReading(levelM: number, hoursAgo = 0, stationId = "test-station"): GaugeReading {
...
function makeForecast(rainfallMm: number): WeatherForecast {
  const now = new Date();
  return {
...
describe("scoreGaugeLevel", () => {
  ...
  it("score increases monotonically with level", () => {
    ...
    for (let i = 1; i < scores.length; i++) {
      ...
      expect(scores[i]).toBeGreaterThanOrEqual(prev);
    }
  });
});
...
describe("scoreGaugeTrend", () => {
  ...
  it("returns stable when change is negligible", () => {
    ...
    const r1 = makeReading(5.0, 1); // 0.01 m/h < 0.05 threshold
    const { signals } = scoreGaugeTrend([r0, r1]);
    expect(signals.some((s) => s.code === "TREND_STABLE")).toBe(true);
  });
});
